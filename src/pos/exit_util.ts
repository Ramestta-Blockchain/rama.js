...
export class ExitUtil {
    private ramaClient_: BaseWeb3Client;
    ...
    getChainBlockInfo(burnTxHash: string) {
        return Promise.all([
            ...
            this.ramaClient_.getTransaction(burnTxHash),
        ]).then(result => {
            return {
    ...
    private getRootBlockInfoFromAPI(txBlockNumber: number) {
        this.ramaClient_.logger.log("block info from API 1");
        return service.network.getBlockIncluded(
            this.config.network,
            txBlockNumber
        ).then(headerBlock => {
            this.ramaClient_.logger.log("block info from API 2", headerBlock);
            if (!headerBlock || !headerBlock.start || !headerBlock.end || !headerBlock.headerBlockNumber) {
                throw Error('Network API Error');
            }
            return headerBlock;
        }).catch(err => {
            this.ramaClient_.logger.log("block info from API", err);
    ...
    private getBlockProofFromAPI(txBlockNumber: number, rootBlockInfo: { start, end }) {
        ...
        return service.network.getProof(
            ...
            this.ramaClient_.logger.log("block proof from API 1");
            return blockProof;
        }).catch(_ => {
    ...
    buildPayloadForExit(burnTxHash: string, logEventSig: string, isFast: boolean, index = 0) {
        ...
        return this.getChainBlockInfo(
            ...
            return Promise.all([
                this.ramaClient_.getTransactionReceipt(burnTxHash),
                this.ramaClient_.getBlockWithTransaction(txBlockNumber)
            ]);
        }).then(result => {
    ...
    getExitHash(burnTxHash, index, logEventSig) {
        ...
        return Promise.all([
            ...
            lastChildBlock = result[0];
            receipt = result[1];
            return this.ramaClient_.getBlockWithTransaction(
                receipt.blockNumber
            );
        }).then(blockResult => {
            block = blockResult;
            if (!this.isCheckPointed_({ lastChildBlock: lastChildBlock, txBlockNumber: receipt.blockNumber })) {
                this.ramaClient_.logger.error(ERROR_TYPE.BurnTxNotCheckPointed).throw();
            }
            return ProofUtil.getReceiptProof(
                receipt,
                block,
            ...
            return this.ramaClient_.etheriumSha3(
